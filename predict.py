...
# picam2.set_controls({"AfMode": controls.AfModeEnum.Auto,"AfTrigger": 1})
    ...
    while ACTIVE:
        ...
        if button_state != prev_state:
            if(button_state == GPIO.LOW):
                MAX_SHUT-=1
                print("starting autofocus cycle")
                # picam2.set_controls({"AfMode": 1,"AfTrigger": 0})
                # picam2.set_controls({"AfMode": 0, "LensPosition": 7})
                # while(not picam2.autofocus_cycle(wait=True)):
                # while(not picam2.autofocus_cycle()):
                    # print("...cycle")

                    
                print("Finished autofocus cycle")
